  ...
  public:
    Impl(int device, const FrameInfo& info, const Params& params) :
        ...
        for (auto i = 0; kernels[i] != ""; ++i) {
            kernels_.emplace(
                kernels[i], cl::Kernel(program, kernels[i].data()));
        }

        input_ = cl::Buffer(
            context, CL_MEM_ALLOC_HOST_PTR, std::max(outputSize_, info.size()));
        rgb_ = cl::Buffer(
            context, CL_MEM_USE_HOST_PTR, outputSize_, rgbBuffer_.data());

        gsimage_ = cl::Image2D(context, CL_MEM_READ_WRITE,
            cl::ImageFormat(CL_R, CL_UNSIGNED_INT8), info_.width, info_.height);

        filtered_ = cl::Image2D(context, CL_MEM_READ_WRITE,
            cl::ImageFormat(CL_R, CL_UNSIGNED_INT8), info_.width, info_.height);

        features_ = cl::Image2D(context, CL_MEM_READ_WRITE,
            cl::ImageFormat(CL_R, CL_UNSIGNED_INT16), info_.width,
            info_.height);
    }

    template <class Destination, class Range>
    void convertOperation(cl::Kernel& convertKernel, const Range& global,
        ...
        if (ret != CL_SUCCESS) {
            std::cout << "conversion error " << ret << std::endl;
        }
    }
    ...
    Frame compute(const Frame& input) {
        ...
        queue_.enqueueWriteBuffer(
            input_, CL_TRUE, 0, input.size(), input.data());

        switch (info_.format) {
            case FrameFormat::RGB:
                queue_.enqueueWriteBuffer(
                    rgb_, CL_TRUE, 0, input.size(), input.data());
                break;

            case FrameFormat::YUYV:
                // 4 yuyv bytes = 2 rgb pixels => iteration count = w * h / 2
                convertOperation(kernels_["yuyv2rgb"], square_ >> 1, 1u, rgb_);
                convertOperation(kernels_["yuyv2gs"],
        ...
        queue_.enqueueNDRangeKernel(kernels_["median"], cl::NullRange,
            cl::NDRange(info_.width, info_.height), cl::NDRange(1, 1));

        // detect corners
        kernels_["fast"].setArg(0, filtered_);
        kernels_["fast"].setArg(1, features_);
        queue_.enqueueNDRangeKernel(kernels_["fast"], cl::NDRange(3, 3),
            cl::NDRange(info_.width - 6, info_.height - 6), cl::NDRange(1, 1));

        // draw corners
        kernels_["draw"].setArg(0, features_);
        kernels_["draw"].setArg(1, rgb_);
        queue_.enqueueNDRangeKernel(kernels_["draw"], cl::NullRange,
            cl::NDRange(info_.width, info_.height), cl::NDRange(1, 1));

        queue_.enqueueReadBuffer(
            rgb_, CL_TRUE, 0, rgbBuffer_.size(), rgbBuffer_.data());
        queue_.finish();
